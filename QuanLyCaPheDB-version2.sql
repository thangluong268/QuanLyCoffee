...
----------------Constraint--------------------
alter table NhanVien add constraint check_SDT_NhanVien 
check (SDT not like '%[^0-9]%' and (len(SDT) = 10))
go

alter table NhanVien add constraint check_ChucVu_NhanVien
check (ChucVu like N'Thu ngân' or ChucVu like N'Pha chế' 
	or ChucVu like N'Phục vụ' or ChucVu like N'Quản lý')
go

alter table NhanVien add constraint check_MaNV
check (MaNV like 'NV%')
go

alter table Ban add constraint check_MaBan
check (MaBan like 'MB%')
go

alter table LoaiMon add constraint check_MaLoai
check (MaLoai like 'ML%')
go

alter table ThucDon add constraint check_MaMon
check (MaMon like 'MM%')
go
...
-- Trigger tạo/xóa tài khoản khi thêm nhân viên mới hoặc sửa nhân viên
...
begin
 ...
 go
  ...
  -- Trigger cập nhật trạng thái của thực đơn = true khi sửa trạng thái của loại món = true
...
begin
	begin transaction
		...
		-- revoke là thu hồi
		-- function:
		-- + nếu function trả về giá trị (ko phải bảng) -> exec
		-- + nếu function trả về bảng) -> select
		-- procedure -> exec
...
-- Load tính lương nhân viên sử dụng view TinhLuong_View
create procedure p_LoadTinhLuong
as
	begin
		select * from TinhLuong_View
	end
go


-- Đặt số ngày làm = 0 khi tính lương
create procedure p_TinhLuong
as
	begin
		update NhanVien set SoNgayLam = 0
	end
go
...
go
exec p_ThemNhanVien 'NV04', NV04, N'Đỗ Thị Nhung', N'22/18 Linh Chiểu, Thủ Đức','0355849046', '2021-03-01', 30, 1, '1996-09-22', N'Pha chế', 'MC04', 1
go
exec p_ThemNhanVien 'NV05', NV05, N'Trần Thị Thùy', N'35/12 Linh Chiểu, Thủ Đức','0987655624', '2021-03-01', 30, 1, '1999-01-03', N'Phục vụ', 'MC05', 1
go
...
go
exec p_ThemNhanVien 'NV08', NV08, N'Tôn Thất Gia', N'57/85 Linh Chiểu, Thủ Đức','0399857845', '2021-03-01', 30, 0, '1995-05-28', N'Pha chế', 'MC08', 2
go
...
go
exec p_ThemNhanVien 'NV13', NV13, N'Trần Thị Trúc', N'11/57 Bình Thọ, Thủ Đức','0383522154', '2021-03-01', 30, 0, '1995-09-09', N'Phục vụ', 'MC13', 3
go
